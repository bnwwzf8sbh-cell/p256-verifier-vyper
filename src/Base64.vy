...
def _encode(data: Bytes[_DATA_INPUT_BOUND], base64_url: bool) -> DynArray[String[4], _DATA_OUTPUT_BOUND]:
    ...
    empty2: Bytes[2] = empty(Bytes[2])
    empty1: Bytes[1] = empty(Bytes[1])
    data_padded: Bytes[_DATA_INPUT_BOUND + 2] = b""
    if padding == 1:
        data_padded = concat(data, empty2)
    elif padding == 2:
        data_padded = concat(data, empty1)
    else:
        data_padded = data
    ...
    if padding == 1:
        last_chunk: String[4] = char_chunks.pop()
        lc2: String[2] = slice(last_chunk, empty(uint256), 2)
        char_chunks.append(concat(lc2, "=="))
    # Case 2: padding of "=" added.
    elif padding == 2:
        last_chunk: String[4] = char_chunks.pop()
        lc3: String[3] = slice(last_chunk, empty(uint256), 3)
        char_chunks.append(concat(lc3, "="))
...
def _index_of(char: String[1], base64_url: bool) -> uint256:
    ...
    assert pos < 65, "base64: invalid string"
